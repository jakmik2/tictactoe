...
impl Game {
    ...
    pub fn play_game(&mut self) {
        ...
        self.num_players = self.additional_player();
        while !finished {
            self.play_turn();
    ...
    fn additional_player(&self) -> u32 {
        loop {
            let mut player_input = String::new();

            println!(
                "\nHow many humans are playing (1 or 2)?: ");

            match io::stdin().read_line(&mut player_input) {
                Err(_) => println!(
                    "error reading input, try again!"),
                Ok(_) => match self.validate_player(&*player_input) {
                    Err(err) => println!("{}", err),
                    Ok(num) => return num,
                },
            }
    ...
    fn validate_player(&self, input: &str) -> Result<u32, String> {
        match input.trim().parse::<u32>() {
            ...
            Ok(number) => {
                if number == 1 || number == 2 {
                    ...
                    Err(
                        String::from("Please input either 1 or 2!"))
                }
            }
        }
    }
        ...
        match input.trim().parse::<u32>() {
            ...
            Ok(number) => {
                if self.is_valid_move(number) {
                    ...
                    Err(
                        String::from("Please input a number, between \
                        1 and 9, not alread chosen!"))
                }
            }
        }
    }
